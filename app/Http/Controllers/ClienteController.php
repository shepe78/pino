...
{  
    ...
    public function index( Request $request)
    {
        ...
        $clientes= Cliente::with(['plaza','giro','ciudad','grupo','analista','asesor','promotor'])
        ...
        ->paginate(15);
         ...
         $ca = Cliente::where(['status' => 'Activo'])->get()->count();
         $ci = Cliente::where(['status' => 'Inactivo'])->get()->count();
         $cp = Cliente::where(['status' => 'Prospecto'])->get()->count();

        
        return view('clientes.index')->with(compact('clientes','registros', 'ca','ci','cp'));
    }
